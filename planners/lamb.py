...
class Lamb(Planner):
    ...
    def move_out_backchain(self, q_start, q_goal, p_through, v_0, H=None, attachment=None,
        ...
        for attach_pose in attachment_poses:
            ...
            while True:
                ...
                p_place = self.lamb(attach_pose, q_place, v_p, attachment=[obj_obstruction, grasp, obj],
                                    obstructions=obstructions,
                                    wrong_placements=wrong_placements.union(p_through_voxels),
                                    enforced_obstacles=enforced_obstacles)
                if p_place is None:
                ...
                # attached
                obj_oobb_placed = self.env.movable_object_oobb_from_q(obj_obstruction, p_place[-1][0], grasp)
                self.env.movable_boxes.append(obj_oobb_placed)
                if attachment is not None:
                    p_goal = self.lamb(p_place[-1][0], q_start, v_p, obstructions=obstructions,
                                   enforced_obstacles=enforced_obstacles,
                                   wrong_placements=wrong_placements)
                else:
    ...
    def a_star(self, q_start, q_goal, v_0, H=None, relaxed=False, obstructions=set(),
        ...
        try:
            ...
            print("Search Time: {}".format(done))
            if self.debug:
                self.G.plot_search(self.env, extended, goal=q_goal, enforced_obstacles=enforced_obstacles, R=obstructions)
            return None
