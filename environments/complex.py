...
from pybullet_planning.pybullet_tools.utils import (set_pose, set_joint_position, Pose, Point,
                                                    load_model, create_box, TAN, GREY, BROWN)
import random
import math
...
class Complex(Environment):
    ...
    def create_room(self):
        ...
        floor2 = self.create_pillar(width=2, length=3, color=TAN)
        ...
        wall_2 = self.create_pillar(width=2, length=wall_thickness, height=wall_height, color=GREY)
        set_pose(wall_2, Pose(point=Point(x=0, y=center[1]-(length/2+wall_thickness/2), z=wall_height/2)))
        
        wall_3 = self.create_pillar(width=2, length=wall_thickness, height=wall_height, color=GREY)
        ...
        wall_5 = self.create_pillar(length=length, width=wall_thickness, height=wall_height, color=GREY)
        set_pose(wall_5, Pose(point=Point(y=center[1], x=center[0]-(width/2+wall_thickness/2), z=wall_height/2)))

        wall_6 = self.create_pillar(width=2, length=wall_thickness, height=wall_height, color=GREY)
        set_pose(wall_6, Pose(point=Point(x=2, y=center[1]-(length/2+wall_thickness/2+3), z=wall_height/2)))

        wall_7 = self.create_pillar(length=3, width=wall_thickness, height=wall_height, color=GREY)
        set_pose(wall_7, Pose(point=Point(y=-3.5, x=1-wall_thickness/2, z=wall_height/2)))

        wall_8 = self.create_pillar(length=3, width=wall_thickness, height=wall_height, color=GREY)
        set_pose(wall_8, Pose(point=Point(y=-3.5, x=3+wall_thickness/2, z=wall_height/2)))


        return {"floor": [floor1, floor2],
